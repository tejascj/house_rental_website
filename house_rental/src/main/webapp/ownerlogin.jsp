...
try {
	...
	if (request.getParameter("submit") != null) //check login button click event not null
	{
		...
		pstmt = con.prepareStatement("select * from ownerlogin where username=? AND password=?"); //sql select query 
	...
	if (username.equals(dbusername) && password.equals(dbpassword)) {
		session.setAttribute("owner", dbusername); //session name is login and store fetchable database email address
		response.sendRedirect("ownerhome.jsp"); //after login success redirect to welcome.jsp page
	}
		} else {
				...
				<div class="login-form">
					...
					<form method="post">
						<div class="input-boxes">
							...
							<a href="ownerreg.jsp">Don't have an account? <label>Sigup
									now</label></a>
						</div>
					</form>
				</div>
